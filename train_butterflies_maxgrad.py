...
def main():
    ...
    if not os.path.exists('./datasets/butterflies_crop/' + args.selection):
        os.makedirs('./datasets/butterflies_crop/' + args.selection)
    ...
    for iter in range(args.iters):
        ...
        if iter == 0:
            ...
            assert callable(datasets_mt.__dict__['butterflies'])
            get_dataset = getattr(datasets_mt, 'butterflies')
            num_classes = datasets_mt._NUM_CLASSES['butterflies']
            train_loader, val_loader = get_dataset(
            ...
            assert callable(datasets_mt.__dict__['butterflies_Dt'])
            get_dataset = getattr(datasets_mt, 'butterflies_Dt')
            num_classes = datasets_mt._NUM_CLASSES['butterflies_Dt']
            ...
            for k in range(len(added_example_indices)):
                ...
                teaching_example_index = teaching_example_index + 1
            fl.close()
            assert callable(datasets_mt.__dict__['butterflies_Lt'])
            get_dataset = getattr(datasets_mt, 'butterflies_Lt')
            num_classes = datasets_mt._NUM_CLASSES['butterflies_Lt']
    ...
    if not os.path.exists('./butterflies'):
        os.makedirs('./butterflies')
    ...
    plt.ylabel('accuracy')
    plt.savefig('./butterflies/evolution_' + args.selection + '.jpg')
